...
: parent_node_(parent_node)
{
  ...
  parent_node_->declare_parameter("tf_frame_camera_", "head_front_camera_link");
  parent_node_->declare_parameter("tf_frame_map_", "map");
  ...
  if (status) {
    if (interests_.find(id) == interests_.end()) {
      ...
      if (id.find("person") != std::string::npos) {
        parent_node_->add_activation("perception_people_detection_node");
      } else {
        parent_node_->add_activation("perception_objects_detection_node");
      }
    ...
    interests_.erase(id);
    if (id.find("person") != std::string::npos) {
      parent_node_->remove_activation("perception_people_detection_node");
    } else {
      parent_node_->remove_activation("perception_objects_detection_node");
    }
